...
for points in troute:
    ax.plot(madison.loc[points]['x'],madison.loc[points]['y'],markersize=20,c=clist[cnt],label=str(cnt),zorder=30)
    cnt+=1
ax.plot(madison.loc[troute[0][0]]['x'],madison.loc[troute[0][0]]['y'],'o',markersize=30,label='Cento',c='r')

ax.plot(madison.x.values[targets],madison.y.values[targets],'x',markersize=20,c='g')
#ax.legend(loc='best')
